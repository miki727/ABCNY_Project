...
for index, row in ofac_list_sampled.iterrows():
    ...
    while True:
        ...
        if num_of_alphabet(replace_word) >= 6:
            break
    ...
    if len(replace_word_list) - rand_letter_index == 4:
        random_length = 4
    else:
        random_length = np.random.choice(list(range(4, len(replace_word_list) - rand_letter_index, 2)))

    for i in range(0, random_length + 1, 2):
